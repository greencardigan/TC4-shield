...
//note, the profile format is as follows: 
    ...
    for (int i=0; i < (lines_pf.length); i++) {		
      ...
      if (lines_pf[i].length() != 0)  {   //check for 0 length lines
        if (lines_pf[i].charAt(0) != '/')  {   // a "/" indicates the line is a comment line,so ignore it.
          ...
          if (prof_found == true) {
            if (i == (start_prof + 1)) {    //check for first line in this profile, this line contains profile number
              char index_s[] = {'0','0'}; 
            ...
            else if (i == (start_prof + 2)) {    //check for 2nd line, this line contains profile name
              int k;
              ...
              k++;  
              while (k<17) {  //if name is less then 17 chars long, send spaces until 17 chars are sent
