...
// also displays the Juice box (Variac) reading
...
PrintWriter logfile;
String appname = "Gesha Roast Logger v1.0";

String cfgfilename = "pGesha.cfg"; // whichport, baudrate
...
void serialEvent(Serial comport) {
    ...
    // ET
    T2[0][idx] = timestamp;
    T2[1][idx] = float(rec[2]);

    // Juice
    T3[0][idx] = timestamp;
    T3[1][idx] = float(rec[4]);
  
    for (int i=0; i<5; i++) {
      print(rec[i]);
      logfile.print(rec[i]);
      if (i < 4) print(",");
      if (i < 4) logfile.print(",");
    }
